...
class Author(models.Model):
    ...
    def update_rating(self):
        self.rating = 0
        aut_post = Post.objects.filter(author=self.author_acc)
        r1 = aut_post.aggregate(Sum('rating'))
        r1 = r1.get(str(list(r1.keys())[0]))
        if r1 is None:
            r1 = 0
        aut_comm = Comment.objects.filter(user=self.author_acc)
        r2 = aut_comm.aggregate(Sum('rating'))
        r2 = r2.get(str(list(r2.keys())[0]))
        if r2 is None:
            r2 = 0
        all_com = Comment.objects.filter(post__author=self.author_acc).values('user', 'rating')
        r3 = all_com.aggregate(Sum('rating'))
        r3 = r3.get(str(list(r3.keys())[0]))
        if r3 is None:
            r3 = 0
        ex_com = Comment.objects.filter(user=self.author_acc, post__author=self.author_acc).values('user', 'rating')
